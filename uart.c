...
int ReadIntegerFromUART(void) {
    char inputString[10] = {0}; // Buffer to store input characters, initialized to all zeros
    int index = 0; // Index to keep track of the next position in the buffer
    char inputChar; // Variable to hold each character read from UART

    // Read characters until newline (Enter key) is hit or the buffer is about to overflow
    while (index < sizeof(inputString) - 1) { // Leave space for the null terminator
        inputChar = UART_InChar(); // Read a character from UART

        UART_OutChar(inputChar); // Echo the character back to UART for user feedback

        if (inputChar == '\r' || inputChar == '\n') { // If Enter key is pressed ('\r' for carriage return)
            break; // Exit the loop
        }
        inputString[index++] = inputChar; // Store the character and move to the next position
    }

    inputString[index] = '\0'; // Null-terminate the string

    return atoi(inputString); // Convert the string to an integer and return it
}
